...
| D6 | `scripts/atlas65/setup-gbrain.mjs` | Plan's top-level script restructured into `main()` + `FAILED` sentinel + guarded `sh()` helper (exit 2 for clone/install/pin technical failures, new `E_GBRAIN_INSTALL`); review round 2 added a torn-state guard (`E_GBRAIN_TORN_STATE` when config.json exists but brain.pglite is missing — upstream gbrain would silently fabricate an empty brain), pin-mismatch remediation hint; `gbrain-cli.mjs` error surfaces keep evidence (stdout slice in callOp parse error, tail-slice stderr) | D2 idiom applied consistently; fail-visible over fail-silent for the persistence foundation Tasks 6/7 stand on. callOp's whole-stdout JSON parse verified correct against the pinned gbrain source (single `console.log(JSON.stringify(...))` in `call`; pretty-printed multi-line output makes a last-line parse actively wrong). |

| D7 | `scripts/atlas65/import-to-gbrain.mjs` (plan repair, review round on Task 6) | (a) `sources_list`/`sources_add` run WITHOUT `GBRAIN_SOURCE` — the pinned `gbrain call` strictly resolves the env source before dispatching any op, so the plan's scoped registration calls would fail on every fresh brain (Critical, plan-inherited); (b) registration check is structural (parsed sources ids, unrecognizable shape = hard failure) instead of JSON-substring, followed by a SCOPED `sources_list` probe that converts a wrongly-skipped registration into a hard failure before the first page write (gbrain's `makeContext` silently falls back to the `default` source for non-explicit tiers — a wrong skip would otherwise import into the wrong scope with exit 0); (c) receipt's checkout-HEAD read is status-checked; (d) capture `schema_version` pinned to "1.0". Additionally the capture gains a structural validation stage (`E_CAPTURE_INVALID`) after the scope gate. | First-contact failure and silent wrong-scope both verified against the pinned gbrain source (`src/commands/call.ts:51` strict resolve; `src/cli.ts:984` makeContext fallback; `sources-ops.ts` collision throw — no upsert). Import remains additive-only across capture generations (documented property; same-capture re-runs verified idempotent: put upsert, links UNIQUE NULLS NOT DISTINCT + ON CONFLICT DO UPDATE). |

| D8 | `scripts/atlas65/generate-snapshot.mjs` (Task 7) | D2/D6-style idiom restructure (main() + FAILED sentinel, no process.exit); the plan's `?? []` shape fallbacks for `list_pages`/`get_links` replaced by fail-closed unrecognized-shape guards (`E_GBRAIN_CLI` with JSON slice) — an unrecognizable CLI shape must never masquerade as empty persistence or zero links; unused `readFileSync` import dropped; persistence gate additionally requires the `brain.pglite` data dir (D6 torn-state consistency) | Same principle as D7's structural check. Readback shapes verified against the pinned checkout: `list_pages` returns a bare array (local callers' explicit limits honored), `get_page` spreads stored custom frontmatter + top-level slug/title, `get_links` rows always carry `from_slug`/`to_slug`/`link_type`/`link_source` (outgoing-only for scoped local callers). |
| D9 | Task 8 (pre-implementation amendment) | Provenance sidecar gains `generated_at` (passed into `buildSnapshotFromReadback` as a parameter — the module stays pure) and both are written tmp+rename after the contract gate; generator guards `pageRows.length >= 500` as a truncation fail-stop; the viewer/server surface `generated_at`. | Review of Task 7 found the stale-snapshot window: a failed re-generation leaves the previous (still contract-valid) snapshot+provenance being served as current. Freshness lives in the sidecar because the snapshot schema is closed (`E_UNKNOWN_FIELD`). |

| D10 | `scripts/atlas65/serve.mjs`, `generate-snapshot.mjs`, viewer, serve tests (Task 8 review round) | Startup requires the request+snapshot+provenance triple (torn publish refused); request handler try/catch → 500 `E_SERVE_IO` (single bad read never kills the server, `headersSent` guard); `/graph-snapshot.json` serves the literally-validated bytes (buffer → tmp file → contract CLI → serve that buffer; tmp unlinked in finally); truncation guard on RAW `list_pages` count before the `pages/` filter; viewer error banner via createElement/textContent, visible "provenance sidecar unavailable" note, `https:`-gated links; +2 serve tests (sidecar 200, torn-startup refusal) | Review found: request-time throw killed the whole server with a misleading browser symptom; filtered-count guard could miss truncation padded by non-pages slugs; validate-then-reread TOCTOU falsified the module's served-bytes claim. Accepted residuals documented: microsecond snapshot/provenance rename window; mixed-generation viewer fetches during regeneration (self-heal on refresh). |

| D11 | `scripts/atlas65/e2e.mjs` (Task 9) | D2-style idiom restructure; NEW gates beyond the plan's block: `E_CONTRACT_EVIDENCE` (re-asserts the generator's contract-response `valid === true`), `E_MEMBERSHIP_MISMATCH` (set equality of this run's `capture.pages[].page_id` vs `snapshot.nodes[].source_ref` — e2e is the only component that sees both, and the snapshot stage provably cannot read the capture, so equality is a persistence-roundtrip proof, not a tautology); stripped snapshot-stage env forwards `ATLAS65_BUN_BIN` (binary path, not a credential) | Review of Task 9: additive-only imports (D7) could otherwise leak prior-generation pages into committed evidence if the source set ever changes between runs; the bun override would otherwise deterministically break the snapshot stage on machines that need it. Accepted residuals: post-stage snapshot/capture parses unguarded (atomic-rename artifacts of gates that just passed; corrupt read still fails visibly), coherent-but-stale evidence dir when a run fails before the copy block (evidence commit is SUCCESS-coupled). |
