...
**Corrected 2026-08-20 (headed acceptance).** The single-entry note ended `; all are drawn with the same stroke.` in every spelling above. That is an absolute about every edge on the stage, and it is false whenever a node is focused — the ordinary interaction state after any click. `core/scene.mjs:246-249` marks edges incident to the focus `active` and the rest `dim`, and `core/render-webgl.mjs:219-222` strokes those in two different colours AND two different widths. Measured through the shipped shell on the accepted snapshot: nothing focused → edge states `["idle","idle","idle","idle"]`, **1** distinct stroke; one node focused → `["dim","dim","active","active"]`, **2** distinct strokes (`--line-strong` at alpha 0.16 / width 1.25 and `--accent` at alpha 1 / width 2). The legend swatch is `background: var(--line-strong)` at full opacity (`#303845`), so in the focused state it matched **no** edge on the stage while the note asserted uniformity — an encoding AC7 does not admit, because it is not present.

The `>= 2` branch one line down was already correctly scoped (`All relation **types** are drawn with the same stroke`), and the accepted snapshot carries exactly one relation type, so the branch that ships to the Product Owner was precisely the overreaching one. D7 already records that the focus highlight is the only per-edge variation; the sentence did not say it. Both branches now make the same claim, about types, so they cannot disagree.
...
**Corrected 2026-08-20 (headed acceptance).** The rule below spans the legend across the whole stage width, and the base `.a39-legend` rule leaves it at the default `pointer-events: auto`. Together those made the legend a **pointer-capturing lid** over the node overlay: it is a `position: absolute; z-index: 5` sibling of `#stage-host`, so it hit-tests over the `.a39-gnode` buttons, and it holds no interactive content that could need a pointer. Slice 1 could leave it at `auto` — three fixed rows in one corner, `display: none` below 960px; slice 2 changed both halves. Measured headed on the accepted snapshot, default Overview, default zoom, **zero user interaction**, `document.elementFromPoint` at each node button's own centre: 1440×900 legend 315×214 px, **0 of 5** unreachable; 960×700 legend 928×149 px, **2 of 5** report `section.a39-legend`; 900×700 legend 868×149 px, **2 of 5**; 800×700 legend 768×149 px, **2 of 5**; 360×640 legend 328×165 px, **5 of 5** report the legend or its own children. A real mouse click on the root node at 900×700 then left `aria-pressed` empty and `#clear-focus` disabled, and a drag across that band moved the stage **0 px** — against the slice-1 baseline `7b88d4f0` at identical coordinates, which focused the node and panned **110 px**.

This is the identical failure mode as DEFECT A, one element over, and it reached the user on **first load** rather than after an interaction. The author gave `.a39-stage-controlbar` `pointer-events: none` for exactly this reason and did not carry the reasoning to the legend, whose footprint the same commit grew from zero to 868×149 px. `display: none` is not available (AC7 needs the legend visible at the countercheck viewport, and the shell contract forbids it), so the base rule gains `pointer-events: none`. Re-measured after the change: **0 of 5** unreachable at every viewport where the legend was the cause, and the click and pan proofs both match the slice-1 baseline exactly. The only viewport still reporting occlusion is 480×300, where the **footer** covers the squeezed stage — reproduced identically on the slice-1 baseline, so it is pre-existing and out of this slice's scope. The cost is that legend text can no longer be selected with the mouse; it stays fully available to the keyboard and to assistive technology.
...
**Corrected 2026-08-20 (headed acceptance).** Three claims this slice makes were pinned by **nothing**, and each was reachable by a mutation that left `npm run check` at its exact control score. All three are now assertions in `test/atlas40-shell.test.mjs`, and each was re-measured against the mutation that motivated it.

*The DEFECT A layout repair was unpinned.* The repair is structural — `index.html` wraps both control groups in one `.a39-stage-controlbar`, and `shell.css` lays it out as a single non-wrapping flex line and strips the per-group absolute anchoring — and nothing in `test/`, `scripts/` or `docs/` referenced `a39-stage-controlbar` at all. Measured: reverting `viewer/atlas39/index.html` and `viewer/atlas39/shell.css` to `74ab1e7e` restores the exact pre-repair defect and scored `CHECK_EXIT:0`, `ℹ tests 498 / pass 498 / fail 0`, `VALIDATION PASSED` — identical to the control run at HEAD. That is the failure mode this branch documents as its own core lesson in `core/gesture.mjs:1-14`. The new test *the two stage-control groups share one flex line and neither anchors itself* pins the DOM structure (both groups inside the bar, none outside it) and the CSS invariants (`display: flex`, `flex-wrap: nowrap`, `pointer-events: none` on the bar; no `position`/`left`/`right`/`inset` on any of `.a39-stage-controls`, `.a39-view-controls`, `.a39-zoom-controls`, read over the **whole** stylesheet). Re-measured: the full revert now scores `CHECK_EXIT:1` with that test red, the HTML-only revert scores exit 1 with the same test red, and the four targeted mutants — `flex-wrap: wrap`, the bar's `pointer-events` removed, `position: absolute` re-added to `.a39-view-controls` — each score `pass 48 / fail 1` on the same test.

*The live region could announce success over a torn-down stage.* `announce()` was an unconditional `dom.live.textContent = message`, and every state-changing handler announces **after** `render()` — which can tear the stage down mid-handler through `paintStage()`'s try/catch. Measured headed on the accepted snapshot with a GL context that starts fine and throws from `drawArrays` later: `body[data-stage]=failed`, `#saved-view-state=""`, `#view-readout="—"`, legend emptied, and `#live` reading **"Saved view restored. Direct neighbourhood of “Sprint 2 – Visible Real Semantic Atlas – Sprint Plan” — 2 of 5 nodes, 1 of 4 relations."** The same over `setView` ("Direct neighbourhood of …") and `setFocus` ("… focused. Level 2. 1 direct relation."). That is a restore shown as successful when the system refused it — what rule 2 of `app.mjs`'s header forbids — reaching the one channel an assistive-technology user has. `announce()` now returns early on `state.failed`, `showFailure` speaks on a separate unconditional `announceFailure()`, and the new test asserts the guard, the `announceFailure` call, and that the live region has **exactly two** writers over comment-free code so the guard cannot be walked past. Re-measured headed: all three routes now announce `"Renderer failed The WebGL stage could not draw the graph: …"`.

*The legend was a pointer-capturing lid over the stage.* See the correction in Task 6, Step 2. The new test *the legend is a decoration, not a lid over the stage hit targets* asserts `pointer-events: none` on the panel, that no part of it (`.a39-legend`, `-row`, `-note`, `-title`, `-group`, `.a39-edge-swatch`, `.a39-swatch`) takes the pointer back, and — because giving up the pointer is only safe while the panel needs none — that neither the markup nor `paintLegend` builds an interactive or focusable element.

**Corrected 2026-08-20 (headed acceptance).** *The restore's visible line told half the truth.* `onRestoreView` wrote `dom.savedViewState.textContent = 'Restored.'` on both sides of the `leftView` branch, so when the saved view was **discarded** — its focus is one the saved view does not draw, a path this shell treats as real because stored text is untrusted — the sighted user read a flat `Restored.` beside an **Overview** readout while the stored record named a neighbourhood. Only the live region said so. Measured headed with the record `{mode:'neighbourhood', anchor_id:…:22478849, focus_id:…:14778372}`: `#saved-view-state="Restored."`, `body[data-saved-view]=restored`, `#view-readout="Overview — 5 of 5 nodes, 4 of 4 relations."`, `#live="Saved view restored. Overview — … The saved focus is not drawn by the saved view, so the whole graph is shown instead."` That is the mirror image of the `setFocus` defect this slice already corrected, where only the assistive-technology user got the wrong sentence. The line now reads `Restored without the saved view: its focus is not drawn by it.` on that branch and `Restored.` on the other; the re-fit case is deliberately unchanged, because D6 assigns that sentence to the announcement. Re-measured headed: exact restore `"Restored."`, discarded-view restore `"Restored without the saved view: its focus is not drawn by it."`, and the mutant that flattens the line again scores `pass 48 / fail 1`.
...
**Corrected 2026-08-20 (headed acceptance).** *The seventh check's name, again.* The paragraph directly above says "the name is corrected to the shapes the condition actually looks for". It was not, and the sentence claiming it was itself the defect. The name it produced — `carry no p95, FPS or millisecond figure outside DEC-07's target sentence` — still promised a CONCEPT ("a millisecond figure") over a condition matching three notations, and it silently dropped the fourth (`frames per second`) that the pattern did look for. The evasion cited as the reason for the rename **still passed at that HEAD**. Measured on the regex in isolation: `"40 ms"` true, `"40ms"` true, `"p95"` true, `"60 FPS"` true, `"frames per second"` true, `"40 milliseconds"` **false**, `"sixty frames a second"` **false**, `"holds a steady 60 frames a second"` **false**. End to end, one line inserted into `## Delivered by slice 2` with the disclaimer untouched — `Measured on the accepted snapshot, search returns in 40 milliseconds and pan holds sixty frames a second, so DEC-07 is comfortably met today.` — scored `VALIDATOR_EXIT:0`, `VALIDATION PASSED`, **147** checks, with this check's own ✓ printed over a fabricated measurement.

Renaming alone was what failed last time, so two things change. The PATTERN gains the word forms measured to walk past it — `\bms\b`, `\bmilliseconds?\b`, `\bframes\s+(?:per|an?)\s+seconds?\b` and `\bframes\s*/\s*seconds?\b`. Re-measured after the change, each fabrication alone is red in **both** slice-2 sections: `✗ … : milliseconds`, `✗ … : frames a second`, `✗ … : frames/second`, `✗ … : FPS`, all at `VALIDATOR_EXIT:1`, while the runbook as written stays green at 147 checks and `"the second slice"`, `"a second look"` and `"frames of reference"` do not fire. And the NAME becomes an enumeration of notations rather than a claim about meaning: *the ATLAS-40 slice-2 sections spell none of p95, FPS, ms, millisecond, frames-per-second or frames-a-second outside DEC-07's target sentence*. This check is a **spelling** guard. D10's universal — "No performance claim is made anywhere in this slice" — is not something a regex proves, and the check is no longer named as if it did.
...
| 3 | the legend note reads `Every relation drawn here is parent_of (explicit); the stroke does not vary by relation type.` (corrected 2026-08-20 — see Task 5) |
...
| 27 | **Added 2026-08-20 (headed acceptance):** at **every** acceptance viewport, and on **first load** with no interaction at all, `document.elementFromPoint` at each `.a39-gnode` button's own centre returns that button. Row 22 covers the two control groups only, and that is exactly how the legend shipped as a lid over 2 of the 5 node buttons at every width ≤ 960px — a defect no gate in the repository measured. Assert it again after an ordinary pan, and assert the functional consequence as well: a real mouse click at such a centre focuses the node, and a drag beginning in the legend's band pans the stage |
